...
const createProductReview = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        try {
            ...
            const product = yield product_model_1.default.findById(req.params.id).session(session);
            const curProduct = yield product_model_1.default.find({ _id: req.params.id }).session(session);
            if (product) {
            ...
            if (req.user && product) {
                ...
                const createdReview = new review_model_1.default(review, { session });
                yield createdReview.save({ session });
